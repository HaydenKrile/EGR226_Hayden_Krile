 ...
 */
void main(void){
     int key, holdingCheck, inputs[10000], master[10000];
     int i = 0;
     int j = 0;
     int isMaster = 0;
                     ...
                     case 12:
                         ...
                         if(i > 3){
                             ...
                             if((master[j-4] == inputs[i-4]) & (master[j-3] == inputs[i-3]) & (master[j-2] == inputs[i-2]) & (master[j-1] == inputs[i-1]))
                                 printf("Your codes match!\n[%d][%d][%d][%d]\n[%d][%d][%d][%d]", master[j-4], master[j-3], master[j-2], master[j-1], inputs[i-4], inputs[i-3], inputs[i-2], inputs[i-1]);
             ...
             if(key) {
                 ...
                 switch(key){
                     ...
                     case 12:
                         //if the pound was pressed after four numbers were entered
                         if(j > 3)
                             printf("Your code is: [%d][%d][%d][%d]\n", master[i-4], master[i-3], master[i-2], master[i-1]);
                         //if the pound was pressed before four numbers were entered
                         else{
                             printf("Too few inputs!\n");
                         }
